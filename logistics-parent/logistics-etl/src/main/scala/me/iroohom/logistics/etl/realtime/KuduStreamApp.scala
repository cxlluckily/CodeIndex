package me.iroohom.logistics.etl.realtime

import me.iroohom.logistics.common.beans.logistics.AreasBean
import me.iroohom.logistics.common.beans.parser.{CanalMessageBean, OggMessageBean}
import me.iroohom.logistics.common.{Configuration, SparkUtils, TableMapping, Tools}
import me.iroohom.logistics.etl.parser.DataParser
...
object KuduStreamApp extends StreamApp {
	...
	def saveKuduLogistics(streamDS: Dataset[OggMessageBean]): Unit = {
		// 导入隐式变量所在包
		import me.iroohom.logistics.common.BeanImplicit._
...
/*
		val warehouseSendVehicleDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouseSendVehicle(bean))(WarehouseSendVehicleBeanEncoder)
			.toDF()
		...
		val waybillLineDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWaybillLine(bean))(WaybillLineBeanEncoder)
			.toDF()
		...
		val chargeStandardDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toChargeStandard(bean))(ChargeStandardBeanEncoder)
			.toDF()
		...
		val codesDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toCodes(bean))(CodesBeanEncoder)
			.toDF()
		...
		val collectPackageDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toCollectPackage(bean))(CollectPackageBeanEncoder)
			.toDF()
		...
		val companyDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toCompany(bean))(CompanyBeanEncoder)
			.toDF()
		save(companyDF, TableMapping.COMPANY)
		
		val companyDotMapDF = streamDS
			.filter(bean => bean.getTable == TableMapping.COMPANY_DOT_MAP)
			.map(bean => DataParser.toCompanyDotMap(bean))(CompanyDotMapBeanEncoder)
			.toDF()
		...
		val companyTransportRouteMaDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toCompanyTransportRouteMa(bean))(CompanyTransportRouteMaBeanEncoder)
			.toDF()
		...
		val companyWarehouseMapDF: DataFrame = streamDS.
			...
			.map(bean => DataParser.toCompanyWarehouseMap(bean))(CompanyWarehouseMapBeanEncoder)
			.toDF()
		...
		val consumerSenderInfoDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toConsumerSenderInfo(bean))(ConsumerSenderInfoBeanEncoder)
			.toDF()
		...
		val courierDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toCourier(bean))(CourierBeanEncoder)
			.toDF()
		...
		val deliverPackageDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toDeliverPackage(bean))(DeliverPackageBeanEncoder)
			.toDF()
		...
		val deliverRegionDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toDeliverRegion(bean))(DeliverRegionBeanEncoder)
			.toDF()
		...
		val deliveryRecordDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toDeliveryRecord(bean))(DeliveryRecordBeanEncoder)
			.toDF()
		...
		val dotTransportToolDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toDotTransportTool(bean))(DotTransportToolBeanEncoder)
			.toDF()
		...
		val expressBillDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toExpressBill(bean))(ExpressBillBeanEncoder)
			.toDF()
		...
		val expressPackageDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toExpressPackage(bean))(ExpressPackageBeanEncoder)
			.toDF()
		...
		val fixedAreaDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toFixedArea(bean))(FixedAreaBeanEncoder)
			.toDF()
		...
		val outWarehouseDetailDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toOutWarehouseDetail(bean))(OutWarehouseDetailBeanEncoder)
			.toDF()
		...
		val pushWarehouseDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toPushWarehouse(bean))(PushWarehouseBeanEncoder)
			.toDF()
		...
		val pushWarehouseDetailDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toPushWarehouseDetail(bean))(PushWarehouseDetailBeanEncoder)
			.toDF()
		...
		val routeDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toRoute(bean))(RouteBeanEncoder)
			.toDF()
		...
		val serviceEvaluationDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toServiceEvaluation(bean))(ServiceEvaluationBeanEncoder)
			.toDF()
		...
		val storeGridDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toStoreGrid(bean))(StoreGridBeanEncoder)
			.toDF()
		...
		val transportToolDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toTransportTool(bean))(TransportToolBeanEncoder)
			.toDF()
		...
		val vehicleMonitorDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toVehicleMonitor(bean))(VehicleMonitorBeanEncoder)
			.toDF()
		...
		val warehouseDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouse(bean))(WarehouseBeanEncoder)
			.toDF()
		...
		val warehouseEmpDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouseEmp(bean))(WarehouseEmpBeanEncoder)
			.toDF()
		...
		val warehouseRackMapDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouseRackMap(bean))(WarehouseRackMapBeanEncoder)
			.toDF()
		...
		val warehouseReceiptDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouseReceipt(bean))(WarehouseReceiptBeanEncoder)
			.toDF()
		...
		val warehouseReceiptDetailDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouseReceiptDetail(bean))(WarehouseReceiptDetailBeanEncoder)
			.toDF()
		...
		val warehouseTransportToolDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouseTransportTool(bean))(WarehouseTransportToolBeanEncoder)
			.toDF()
		...
		val warehouseVehicleMapDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWarehouseVehicleMap(bean))(WarehouseVehicleMapBeanEncoder)
			.toDF()
		save(warehouseVehicleMapDF, TableMapping.WAREHOUSE_VEHICLE_MAP)
		
		val waybillDF: DataFrame = streamDS
			.filter(bean => bean.getTable == TableMapping.WAYBILL)
			.map(bean => DataParser.toWaybill(bean))(WaybillBeanEncoder)
			.toDF()
		...
		val waybillStateRecordDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWaybillStateRecord(bean))(WaybillStateRecordBeanEncoder)
			.toDF()
		...
		val workTimeDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toWorkTime(bean))(WorkTimeBeanEncoder)
			.toDF()
		...
		val transportRecordDF: DataFrame = streamDS
			...
			.map(bean => DataParser.toTransportRecordBean(bean))(TransportRecordBeanEncoder)
			.toDF()
 ...
 */
	 ...
	 */
	override def save(dataframe: DataFrame, tableName: String,
